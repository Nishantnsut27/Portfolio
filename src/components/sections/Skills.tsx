...
export const Skills = () => {
  ...
  const renderRow = (rowSkills: typeof skills, rowIndex: number) => (
    <div className="flex justify-center gap-4 mb-6" key={rowIndex}>
      {rowSkills.map((skill, idx) => {
        const iconSrc = skillIconMap[skill.icon];
        const isFirst = idx === 0;
        const isLast = idx === rowSkills.length - 1;
        return (
          <motion.article
            key={skill.name}
            ...
            custom={rowIndex * 5 + idx}
            className={`group flex aspect-square w-full flex-col items-center justify-center gap-2 rounded-2xl border border-white/10 bg-gradient-to-br from-slate-800/60 via-slate-700/50 to-slate-800/60 p-4 text-center shadow-[0_8px_20px_rgba(0,0,0,0.4),0_2px_6px_rgba(255,255,255,0.05)_inset] backdrop-blur-sm transition duration-300 hover:scale-105 hover:shadow-[0_12px_32px_rgba(56,189,248,0.18),0_4px_10px_rgba(135,206,250,0.18)] hover:border-sky-400/80 ${isFirst ? 'ml-4' : ''} ${isLast ? 'mr-4' : ''}`}
          >
            <div className="flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-xl bg-white/5">
              {iconSrc ? (
                <img src={iconSrc} alt={`${skill.name} icon`} className="h-10 w-10 sm:h-12 sm:w-12 select-none object-contain" />
              ) : (
                <span className="text-lg font-semibold text-white/70">{skill.name.charAt(0)}</span>
              )}
            </div>
            <h3 className="mt-1 text-xs font-bold text-white tracking-wide leading-tight sm:text-sm">
              {skill.name}
            </h3>
          </motion.article>
        );
